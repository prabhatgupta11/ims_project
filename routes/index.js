...
router.post('/fetchProducts', (req, res) => {
  ...
  // Query the database for product names based on outletId
  // Replace the following line with your actual database query
  const products = queryProductsFromDatabase(outletId);

  res.json(products);
});


function queryProductsFromDatabase(outletId) {
  // Implement your database query here and return the product data as an array of objects
  // For example:
  // const products = [
  //     { itemId: 1, productName: 'Product A' },
  //     { itemId: 2, productName: 'Product B' },
...
router.get('/stockInListData',async function (req, res) {
  ...
  const count = await Order.count({where : {stockType : 'In'}});
...
router.get('/stockOutApprovalList', stockInOutController.stockOutApprovalList);

router.post('/updateStockOutApprovalStatus', stockInOutController.updateStockOutApprovalStatus)
...
router.get('/stockOutListData',async function (req, res) {
  ...
  const count = await Order.count({where : {stockType : 'Out'}});
...
router.get('/stockOutUpdate/:id', async function (req, res) {
  ...
  const store = await Store.findAll()
    const product = await NewProduct.findAll()
    // const supplier = await SupplierMaster.findAll({where : {storeFk : storeIds}})
    res.render('stockInOut/updateStockOut', { title: 'Express', message: req.flash('message'), productPrice,previousStore,order, previousCustomer,store, product });
  });
...
router.get('/stockOutView/:id', async function (req, res) {
  const orderId = req.params.id
  const order = await Order.findOne({where : {rowguid : orderId}})
...
router.get("/stockSummary",async(req,res)=>{
  // const product=await StockInOut.findAll({where:{outletId:  }})
...
//  console.log(888885555,supplier.rows[i].type=="In")
...
let data_arr = [];
for (let i = 0; i < supplier.rows.length; i++) {
 // console.log(7788,supplier.rows[i].type,)

// fetching  product name based on the itemId from stock_ledger

  // const productName = await NewProduct.findOne({
  //   where: { itemId: supplier.rows[i].itemId },
  // });
